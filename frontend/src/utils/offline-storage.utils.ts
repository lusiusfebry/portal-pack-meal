...
export async function getOrdersFromCache(): Promise<Order[]> {
  try {
    const db = await openDB();
    ...
    const result = await new Promise<Order[]>((resolve) => {
      ...
      req.onsuccess = () => {
        const data = (req.result || []) as Order[];
        resolve(data);
      };
      req.onerror = () => {
...
export async function saveUserDataToCache(user: any): Promise<void> {
  try {
...
export async function getUserDataFromCache(): Promise<any | null> {
  try {
    const db = await openDB();
