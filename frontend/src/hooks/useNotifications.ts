...
import type {
  ...
  OrderApprovalRequestedEventPayload,
  OrderApprovalDecidedEventPayload,
} from '@/types/websocket.types';
import type { OrderStatusChangedEventPayload } from '@/types/order.types';
...
export default function useNotifications(): void {
  ...
  useWebSocket(
    ...
    (event: OrderStatusChangedEventPayload) => {
      ...
      const isEmployeeOwner =
        role === 'employee' && user?.karyawanId === event.karyawanPemesanId;
  ...
  useWebSocket(
    ...
    (event: OrderApprovalRequestedEventPayload) => {
      if (role !== 'administrator') {
        ...
        (t) =>
          React.createElement(
            ...
            React.createElement(
              'div',
              { className: 'flex-1' },
              React.createElement('div', { className: 'font-semibold' }, 'Permintaan Approval'),
              React.createElement(
                'div',
                { className: 'text-sm opacity-90' },
                `${event.requestType === 'REJECT' ? 'Penolakan' : 'Edit'} untuk pesanan `,
                React.createElement('span', { className: 'font-mono' }, event.kodePesanan),
                ` (dept ${event.departmentId})`,
              ),
            ),
            React.createElement(
              'button',
              {
                ...
                className:
                  'px-3 py-1 rounded-md text-sm bg-amber-500 hover:bg-amber-600 text-black',
              },
              'Buka',
            ),
          ),
  ...
  useWebSocket(
    ...
    (event: OrderApprovalDecidedEventPayload) => {
