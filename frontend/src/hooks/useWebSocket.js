 ...
 */
export function useWebSocket(event, handler, deps = []) {
    ...
    useEffect(() => {
        ...
        async function ensureTokenAndConnect() {
            ...
            if (token) {
                if (!cancelled)
                    socketManager.connect(token, departmentId);
            }
            else {
    ...
    useEffect(() => {
        socketManager.on(event, memoHandler);
        return () => {
            socketManager.off(event, memoHandler);
        };
