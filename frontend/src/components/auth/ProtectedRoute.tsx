  ...
  if (isLoading || isValidating) {
    return (
      <div className="min-h-[50vh] grid place-items-center">
        <Card className="w-full max-w-sm animate-pulse" padding="md">
          <div className="flex flex-col items-center gap-3 py-6">
            <div
              className="h-10 w-10 rounded-full border-4 border-slate-200 border-t-transparent animate-spin dark:border-slate-700"
              role="status"
              aria-live="polite"
              aria-label="Memuat"
            />
            <p className="text-sm text-slate-600 dark:text-slate-300">Memuat...</p>
          </div>
        </Card>
      </div>
    );
